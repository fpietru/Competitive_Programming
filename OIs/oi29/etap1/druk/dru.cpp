...
int main()
{
	...
	for (auto g : Group)
	{
		...
		for (string p : g.SD)
		{
			...
			else if (psz == P && p.front() == p.back())
			{
				bool f1 = (int)KMP(row1, p).size() * psz <= m;
				bool f2 = (int)KMP(row2, p).size() * psz <= m;
				bool f3 = (int)KMP(col1, p).size() * psz <= n; 
				bool f4 = (int)KMP(col2, p).size() * psz <= n;
	
				if ((n % psz == 0 && (f1 && f2)) || (m % psz == 0 && (f3 && f4)))
					starters.push_back({p, i});
			}
			else if (n % psz == 0 || m % psz == 0)
					starters.push_back({p, i});
